...
export async function getPageContent(id) {
    ...
    try {
        ...
        return { ...PAGE_DEFAULTS[id], ...content };
    }
    catch {
        return PAGE_DEFAULTS[id];
    }
...
// ---- Homams ----
export async function getHomams() {
    if (!canUseSupabase())
...
export async function getFeaturedHomams(limit = 6) {
    if (canUseSupabase()) {
        try {
            const supabase = createSupabaseBrowserClient();
            const { data, error } = await supabase
...
export async function getHomamBySlug(slug) {
    if (canUseSupabase()) {
        try {
            const supabase = createSupabaseBrowserClient();
